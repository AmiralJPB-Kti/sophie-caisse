        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm text-center space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-left">
            <div>
                ...
                <div className="relative"><Mail className="absolute left-3 top-3 text-slate-400" size={20} /><input type="email" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} placeholder="sophie@exemple.com" className="w-full bg-slate-50 border-2 border-slate-200 focus:border-indigo-500 rounded-xl pl-10 pr-4 py-3 outline-none transition-all" autoFocus /></div>
            </div>
            <div>
                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Mot de passe</label>
                <div className="relative"><Lock className="absolute left-3 top-3 text-slate-400" size={20} /><input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} placeholder="••••••••" className="w-full bg-slate-50 border-2 border-slate-200 focus:border-indigo-500 rounded-xl pl-10 pr-4 py-3 outline-none transition-all" /></div>
            </div>
    ...
    <div className="min-h-screen bg-slate-50 pb-20 print:bg-white print:pb-0 relative">
      ...
      <header className="bg-indigo-600 text-white p-6 shadow-lg rounded-b-3xl print:hidden">
        <div className="flex justify-between items-center max-w-md mx-auto">
          <div>
            ...
            <button onClick={() => setShowProfileModal(true)} className="text-xs text-indigo-200 flex items-center gap-1 hover:text-white hover:underline transition mt-1 cursor-pointer"><User size={12} /> {session.user.email} (Modifier)</button>
          </div>
          <div className="flex gap-2">
            <button onClick={window.print} className="bg-white/20 p-2 rounded-full hover:bg-white/30 transition"><Download size={20} /></button>
        ...
        {/* Navigation Mois (Masquée en mode Saisie) */}
      ...
      <main className={`max-w-md mx-auto px-4 ${viewMode === 'table' ? 'max-w-4xl' : ''} ${viewMode === 'stats' ? 'max-w-xl' : ''} -mt-4 print:mt-0 print:max-w-full print:px-0`}>
        {loadingData ? (
          ...
          <>
            {/* VUE SAISIE (Restaurée V1 - Avec icônes couleur) */}
                ...
                {[
                  ...
                  <div key={field.id} className="relative">
                    ...
                    <div className="relative group">
                      {/* L'icône est positionnée en absolu DANS l'input */}
          ...
          <>
            {/* VUE STATISTIQUES (Fixe width issue) */}
